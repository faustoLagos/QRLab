...
class Logger(object):
    ...
    def save_as_csv(self,
        ...
        for i in range(self.NUM_DRONES):
            ...
            with open(csv_dir + "/yawDeg" + str(i) + ".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, list(map(lambda angle: (angle * 180) / np.pi, self.states[i, 9, :]))])), delimiter=",")
            ####
            ...
            with open(csv_dir + "/pwm0-" + str(i) + ".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, (self.states[i, 16, :] - 4070.3) / 0.2685])), delimiter=",")
            with open(csv_dir + "/pwm1-" + str(i) + ".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, (self.states[i, 17, :] - 4070.3) / 0.2685])), delimiter=",")
    ...
    def plot_position_and_orientation(self):
        ...
        row = 0
        # ROLL
        axs[row, col].plot(t, np.rad2deg(self.states[0, 7]), color="tab:blue", linewidth=2)
        axs[row, col].set_xlabel('time')
        ...
        row = 1
        # PITCH
        axs[row, col].plot(t, np.rad2deg(self.states[0, 8]), color="tab:orange", linewidth=2)
        axs[row, col].set_xlabel('time')
        ...
        row = 2
        # YAW
        axs[row, col].plot(t, np.rad2deg(self.states[0, 9]), color="tab:green", linewidth=2)
        axs[row, col].set_xlabel('time')
    ...
    def plot_pwms(self):
        ...
        row = 0
        axs[row, col].plot(t, (self.states[0, 16] - 4070.3)/0.2685, label="drone_" + str(0))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('PWM_0')

        row = 1
        axs[row, col].plot(t, (self.states[0, 17] - 4070.3)/0.2685, label="drone_" + str(0))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('PWM_1')

        col = 1
        row = 0
        axs[row, col].plot(t, (self.states[0, 18] - 4070.3)/0.2685, label="drone_" + str(0))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('PWM_2')

        row = 1
        axs[row, col].plot(t, (self.states[0, 19] - 4070.3)/0.2685, label="drone_" + str(0))
        axs[row, col].set_xlabel('time')
    ...
    def plot_velocities(self):
        ...
        row = 0
        # X-vel
        axs[row, col].plot(t, self.states[0, 3], color="tab:blue", linewidth=2.5)
        # axs[row, col].set_yticklabels(axs[row, col].get_yticks(), weight='bold')
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('$\dot{x} (\\frac{m}{s})$')
        axs[row, col].set_ylim([-1, 1])
        axs[row, col].grid(True)

        row = 1
        # Y-vel
        axs[row, col].plot(t, self.states[0, 4], color="tab:orange", linewidth=2.5)
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('$\dot{y} (\\frac{m}{s})$')
        axs[row, col].set_ylim([-1, 1])
        axs[row, col].grid(True)

        row = 2
        # Z-vel
        axs[row, col].plot(t, self.states[0, 5], color="tab:green", linewidth=2.5)
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('$\dot{z} (\\frac{m}{s})$')
        axs[row, col].grid(True)
        ...
        row = 0
        # ROLL-vel
        axs[row, col].plot(t, np.rad2deg(self.states[0, 13]), color="tab:blue", linewidth=2)
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel("$\dot{\phi} (\\frac{rad}{s})$")
        # axs[row, col].set_ylim([-8.6, 8.6])
        axs[row, col].grid(True)

        row = 1
        # PITCH
        axs[row, col].plot(t, np.rad2deg(self.states[0, 14]), color="tab:orange", linewidth=2)
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel("$\dot{\\theta} (\\frac{rad}{s})$")
#         axs[row, col].set_ylim([-8.6, 8.6])
        axs[row, col].grid(True)

        row = 2
        # YAW
        axs[row, col].plot(t, np.rad2deg(self.states[0, 15]), color="tab:green", linewidth=2)
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel("$\dot{\psi} (\\frac{rad}{s})$")
        axs[row, col].grid(True)
