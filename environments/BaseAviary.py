...
class BaseAviary(gym.Env):
    ...
    def reset(self,
        ...
        #### Housekeeping ##########################################
        self._housekeeping()
        #### Apply reset lifecycle hooks before the first observation
        self._resetModelState(options=options)
        self._resetControllerState(options=options)
        self._resetTaskState(options=options)
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
        #### Start video recording #################################
        self._startVideoRecording()
        ...
        initial_info = dict(self._computeInfo())
        initial_info.update(self._computeResetInfo())
        return initial_obs, initial_info
    
    ################################################################################

    def _resetModelState(self, options: dict | None = None) -> None:
        """Hook for model-level state changes required during reset."""
        pass

    ################################################################################

    def _resetControllerState(self, options: dict | None = None) -> None:
        """Hook for controller-level state changes required during reset."""
        pass

    ################################################################################

    def _resetTaskState(self, options: dict | None = None) -> None:
        """Hook for task-specific state changes required during reset."""
        pass

    ################################################################################

    def _computeResetInfo(self) -> dict:
        """Returns metadata that is specific to an environment reset."""
        return {}
    ...
    @staticmethod
    def _convertWorldVectorToBodyFrame(
            vector_world: np.ndarray,
            quaternion_xyzw: np.ndarray
    ) -> np.ndarray:
        """Converts a vector from world coordinates to body coordinates."""
        rotation_world_from_body = np.asarray(
            p.getMatrixFromQuaternion(quaternion_xyzw),
            dtype=np.float64,
        ).reshape(3, 3)
        vector_world = np.asarray(vector_world, dtype=np.float64).reshape(3)
